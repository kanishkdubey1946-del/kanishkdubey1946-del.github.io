...
const principles = ['Observe the real problem', 'Connect disciplines', 'Build and verify'];
const orbitSkills = [
...
export default function Home(){return <main id="main">
  <section className="hero" aria-labelledby="hero-title">
    ...
    <div className="hero-content"><div className="eyebrow hero-eyebrow"><span className="signal-cross">✳</span> AI · HARDWARE · HUMAN CURIOSITY</div><h1 id="hero-title"><span>Ideas, made</span><span><em>tangible.</em></span></h1><p>{site.person.intro}</p><Link href="#work" className="pill-button">Explore my work <ArrowDown size={17}/></Link></div>
  ...
  <section className="tedy-scroll" aria-labelledby="tedy-title"><div className="tedy-sticky"><div className="tedy-copy"><div className="eyebrow"><span className="section-number">FIELD METHOD</span> FROM SIGNAL TO SYSTEM</div><h2 id="tedy-title">How ideas<br/><em>take shape.</em></h2><p>Each project moves through the same loop: understand the signal, connect the right tools, then build something testable.</p><div className="tedy-steps">{principles.map((item,index)=><article className={`tedy-step ${index===0?'is-active':''}`} key={item}><span>0{index+1}</span><h3>{item}</h3></article>)}</div></div><div className="tedy-orbit-stage" aria-hidden="true"><div className="tedy-orbit"><b className="tedy-core">kd</b><span className="tedy-satellite">ASK</span><span className="tedy-satellite">MODEL</span><span className="tedy-satellite">MAKE</span><span className="tedy-satellite">TEST</span></div></div></div></section>

  <section className="spiral-scroll" aria-labelledby="spiral-title"><div className="spiral-sticky"><div className="spiral-heading"><div><div className="eyebrow"><span className="section-number">ORBIT MAP</span> FIVE CONNECTED PRACTICES</div><h2 id="spiral-title">Systems in <em>motion.</em></h2></div><p>The work is strongest where disciplines overlap. Scroll through the orbit to see the parts of my practice as one connected system.</p></div><div className="spiral-scene">{orbitSkills.map(([label,title,body],index)=><article className="spiral-card" key={label}><span>0{index+1} / {label}</span><h3>{title}</h3><p>{body}</p></article>)}</div></div></section>

  <section className="experience-section section-pad"><div className="section-heading"><div><div className="eyebrow"><span className="section-number">03</span> LEARNING BY DOING</div><h2>Work, and<br/><em>the work around it.</em></h2></div><Link href="/about#experience" className="text-link">Full experience <ArrowUpRight size={18}/></Link></div><div className="experience-list">{site.experience.map(e=><article key={e.title}><span className="experience-period">{e.period}</span><div><h3>{e.title}</h3><p>{e.organization}</p></div><p>{e.body}</p></article>)}</div></section>
