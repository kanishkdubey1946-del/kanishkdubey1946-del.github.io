...
export default function Home(){return <main id="main">
  ...
  <section className="section-pad portfolio-paths" aria-label="Explore the portfolio">{[['01','Projects','AI systems, IoT builds and the code behind them.','/projects'],['02','Achievements','Competition results and research milestones.','/achievements'],['03','Hacke Diaries','The people, places and stories behind the builds.','/hacke-diaries']].map(([number,title,body,url])=><Link key={url} href={url}><span className="eyebrow">{number} / EXPLORE</span><h2>{title}<ArrowUpRight size={24}/></h2><p>{body}</p></Link>)}</section>
  ...
  <section className="recognition-section section-pad" id="recognition"><div className="section-heading"><div><div className="eyebrow"><span className="section-number">04</span> MOMENTS ALONG THE WAY</div><h2>Out in <em>the world.</em></h2></div><Link href="/achievements" className="text-link">All achievements <ArrowUpRight size={18}/></Link></div><div className="recognition-list">{site.recognition.map((r,i)=><article key={r.title}><span className="recognition-index">0{i+1}</span><div><h3>{r.title}</h3><p>{r.body}</p></div><span className="award-result">{r.result}</span><span className="award-date">{r.date}</span></article>)}</div></section>
</main>}
